...
export class UserService {
  ...
  getUserByUsername(username: string): Observable<User[]> {
    return this.http.get<User[]>(`${this.serverUrl}/users?username=${username}`).pipe(
      catchError((error) => {
        return throwError('Failed to retrieve user data');
      })
    );
  ...
  updateUser(userId: number, updatedUser: User): Observable<any> {
    return this.http.put(`${this.serverUrl}/users/${userId}`, updatedUser).pipe(
      tap(() => {
  ...
  updateUserByUsername(username: string, updatedUser: User): Observable<any> {
    return this.http.put(`${this.serverUrl}/users?username=${username}`, updatedUser).pipe(
